...
def train_sampling_rule(X, Y, eta=0.001, max_depth=3, objective='reg:squarederror', boost_rounds=2000, seed=0):
    # Convert to arrays
    X = np.asarray(X)
    Y = np.asarray(Y)
    ...
    tree = xgb.train({'eta': eta, 'max_depth': max_depth, 'objective': objective, 'seed': seed}, dtrain, boost_rounds)
    return tree
...
def perspective_driven_inference(
    estimator,
    Y,
    Yhat,
    sampling_probs,
